...
void initOpengl(void)
{
    ...
    gl.walkImage = ppm6GetImage("./images/Enemy_Mariachi_3.ppm");
...
//animation bug is possibly here
void checkResize(XEvent *e)
{
...
void checkKeys(XEvent *e)
{
	...
	case XK_F:
	    ...
	    gl.walk ^= 1; //should walk to the right
...
void physics(void)
{
    ...
    /* if w or right or left 
     * then start recording
     * advance the frame,
     * */
    if (gl.walk || gl.keys[XK_Right] || gl.keys[XK_Left] || gl.keys[XK_f]) {
	...
	for (int i=0; i<20; i++) {
	    ...
	    if(gl.keys[XK_f] == true){ // .. forever to the RIGHT
		gl.box[i][0] -= 2.0 * (0.05 / gl.delay);
		if (gl.box[i][0] < -10.0)
		    gl.box[i][0] += gl.xres + 10.0;
    ...
    /*
       if(gl.keys[XK_F]){
       timers.recordTime(&timers.timeCurrent);
       double timeSpan2 = timers.timeDiff(&timers.walkTime, &timers.timeCurrent);
...
// my problem isn't in Render
//Render takes care of background, boxes, shadow left and right walk
//source or problem is HOW it is displayed
void render(void)
{
    Rect r;
    //Clear the screen
    ...
    glClear(GL_COLOR_BUFFER_BIT);
    float cx = gl.xres/2.0;  // was 2.0 .. x coordanite
    float cy = gl.yres/1.0; // was 2.0
    //
    //show ground
    glBegin(GL_QUADS);
    //51 , 255, 51
    ...
    //problem is not here
    float h = 200.0;  // was 200
    float w = h * 0.5; ///was .05
    glPushMatrix();
    ...
    if (gl.walkFrame >= 8)
	iy = 1;
    float tx = (float)ix / 7.0; //number of Mariachi's
    float ty = (float)iy / 1.0; //number of ROWS
    ...
    if(gl.keys[XK_Left]){
	//7th
	glTexCoord2f(tx+.125,      .6); glVertex2i(cx-w, cy-h);
	glTexCoord2f(tx+.125,       .0);    glVertex2i(cx-w, cy+h);
	glTexCoord2f(tx, 	    .0);    glVertex2i(cx+w, cy+h);
	glTexCoord2f(tx,  	    .6); glVertex2i(cx+w, cy-h);
    } 
    else {
	/*if(gl.keys[XK_Right]) */ 
	//exp
    ...
    /*else{
	// walk forever ?????? china
	glTexCoord2f(tx,      .5); glVertex2i(cx-w, cy-h);
	glTexCoord2f(tx,      .0);    glVertex2i(cx-w, cy+h);
	glTexCoord2f(tx+.125, .0);    glVertex2i(cx+w, cy+h);
	glTexCoord2f(tx+.125, .5); glVertex2i(cx+w, cy-h);
    }
*/
	...
	r.bot = gl.yres - 20; // was 20
	r.left = 10;
	r.center = 0;
	ggprint8b(&r, 16, c, "F   WALK forever !!!! and eva e e e ");
